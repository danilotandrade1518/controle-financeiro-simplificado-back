...
3. **plan.md**: Plano faseado de implementação (se já existir)
...
**Localização**: Meta Specs estão no caminho configurado em `meta_specs_path` no arquivo `ai.properties.md` na raiz do projeto.
...
- [ ] ARIA attributes conforme padrões do projeto
...
1. **📚 Contexto Inteligente**: Use Context Loading para carregar documentos relevantes (context, architecture)
2. **🧠 Memória Contextual**: Aplique padrões existentes e histórico
3. **⚡ Estratégia Adaptativa**: Use análise de complexidade
4. **🔄 Iterativo**: Trabalhe em pequenas etapas com validação
